...
class Users extends CI_Controller {
        ...
        if ($_SERVER["REQUEST_METHOD"] == 'POST') {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if ($user_id) {
                    ...
                    if ($updated) {
                        $this->session->set_flashdata('success', 'Profile updated successfully.');
                        redirect('pages/db_adminProfile'); // Redirect to db_adminProfile
                    } else {
                        $this->session->set_flashdata('error', 'Failed to update user profile.');
                        redirect('pages/db_authProfile');
                    }
    ...
    public function toggle_status($user_id) {
        ...
        $user = $this->User_model->fetch_user_by_id($user_id);
        $new_status = ($user->status == 1) ? 0 : 1;

        // Update the status
        $this->User_model->update_status($user_id, $new_status);
